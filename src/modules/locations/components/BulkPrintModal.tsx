...
export const BulkPrintModal = ({ isOpen, onClose }: BulkPrintModalProps) => {
  ...
  const filteredLocations = useMemo(() => {
    ...
    return allLocations.filter((l) => {
      if (selectedClientId && l.clientId !== selectedClientId) return false;
      if (selectedZoneId && l.zoneId !== selectedZoneId) return false;
      if (
        searchTerm &&
  ...
  const handlePrint = async () => {
    ...
    try {
      ...
      dispatch(
        showToast({ message: "Error al generar el PDF masivo", type: "error" }),
      );
    } finally {
      setIsPrinting(false);
    }
          ...
          {loading ? (
            ...
            <div className="h-full flex flex-col items-center justify-center text-slate-300 gap-3 opacity-60 py-20">
              ...
              <p className="font-black text-sm uppercase tracking-widest">
                Selecciona un cliente o zona para comenzar
              </p>
            </div>
          ) : filteredLocations.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center text-slate-300 gap-3 opacity-60 py-20">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 pb-4">
              {filteredLocations.map((loc) => (
                ...
                >
                  <div
                    className={`w-5 h-5 rounded-lg border-2 flex items-center justify-center transition-all ${selectedIds.includes(loc.id) ? "bg-emerald-600 border-emerald-600" : "border-slate-200 bg-white"}`}
                  >
                    {selectedIds.includes(loc.id) && (
                      <FaPrint className="text-white text-[10px]" />
                    )}
                  </div>
                  <div className="flex flex-col flex-1">
                    <span
                      className={`text-sm font-black tracking-tight leading-tight ${selectedIds.includes(loc.id) ? "text-slate-900" : "text-slate-500"}`}
                    >
                      {loc.name}
                    </span>
                  </div>
