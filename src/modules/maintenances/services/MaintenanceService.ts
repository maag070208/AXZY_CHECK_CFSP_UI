...
export const getPaginatedMaintenances = async (
...
export const deleteMaintenanceMedia = async (
  maintenanceId: number,
  key: string,
): Promise<TResult<boolean>> => {
  return await remove<boolean>(
    `/maintenance/${maintenanceId}/media?key=${key}`,
  );
};
